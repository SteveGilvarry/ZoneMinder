...
unsigned int Monitor::DetectMotion( const Image &comp_image, Event::StringSet &zoneSet, unsigned int &score )
{
    ...
    for ( int n_zone = 0; n_zone < n_zones; n_zone++ )
    {
        ...
        if ( config.record_diag_images )
        {
            ...
            snprintf( diag_path, sizeof(diag_path), "%s/%d/diag-%d-r.jpg", storage->Path(), id, zone->Id() );
        ...
        {
            static char diag_path[PATH_MAX] = "";
            snprintf( diag_path, sizeof(diag_path), "%s/%d/diag-%d-d.jpg", storage->Path(), id, zone->Id() );
